...
//    USART2_UX->DR = (uint8_t)ch;             /* 将要发送的字符 ch 写入到DR寄存器 */
//    //return ch;
//    }
//#endif
////////////////////////////////////////////////////////////////////////////////////////
...
//    {
//    while ((USART3_UX->SR & 0X40) == 0);     /* 等待上一个字符发送完成 */

//    USART3_UX->DR = (uint8_t)ch;             /* 将要发送的字符 ch 写入到DR寄存器 */
//    return ch;
//    }
//#endif
////////////////////////////////////////////////////////////////////////////////////////
...
char IntermediateBuffer[USART_REC_LEN];//中介字符组
...
uint8_t g_rx_buffer3[RXBUFFERSIZE_3];  /* HAL库使用的串口接收缓冲 */
...
//usart3
    if (huart->Instance == USART3_UX)                    /* 如果是串口2 */
    {
        if ((g_usart_rx_sta3 & 0x8000) == 0)             /* 接收未完成 */
        {
            if (g_usart_rx_sta3 & 0x4000)                /* 接收到了0x0d（即回车键） */
            {
                if (g_rx_buffer3[0] != 0x0a)             /* 接收到的不是0x0a（即不是换行键） */
                {
                    g_usart_rx_sta3 = 0;                 /* 接收错误,重新开始 */
                }
                else                                    /* 接收到的是0x0a（即换行键） */
                {
                    g_usart_rx_sta3 |= 0x8000;           /* 接收完成了 */
                }
            }
            else                                        /* 还没收到0X0d（即回车键） */
            {
                ...
                else
                {
                    ...
                    {
                        g_usart_rx_sta3 = 0;             /* 接收数据错误,重新开始接收 */
                    }
                }
            }
        }
...
{
//    BaseType_t xHigherPriorityTaskWoken = pdFALSE; // 初始值为 pdFALSE
...
void usart_init(uint32_t baudrate,uint32_t baudrate2,uint32_t baudrate3)
{
    ...
    g_uart2_handle.Init.BaudRate = baudrate2;                                  /* 波特率 */
    ...
    g_uart3_handle.Init.BaudRate = baudrate3;                                  /* 波特率 */
...
//usart2/////////////////////////////////////////////////////////        
    if (huart->Instance == USART2_UX)                            /* 如果是串口2，进行串口2 MSP初始化 */
    {
        ...
        HAL_NVIC_SetPriority(USART2_UX_IRQn, 2, 3);              /* 组2，最低优先级:抢占优先级3，子优先级3 */
#endif
    }
...
//usart3//////////////////////////////////////////////////////////        
    if (huart->Instance == USART3_UX)                            /* 如果是串口2，进行串口2 MSP初始化 */
    {
...
#if USART3_EN_RX
        HAL_NVIC_EnableIRQ(USART3_UX_IRQn);                      /* 使能USART2中断通道 */
...
void sendDecimalToHex(char *decimalString) {
    ...
    // 使用 sprintf 将整数格式化为十六进制字符串
    uint8_t hexString[3]={num1,num2,num3};  // 根据实际情况调整数组大小
